...
To save you some potentially long download times, we have cached for you most of the container images required by this workshop; we've done it with the script `$WORK/exercises/prep_pull_images.sh`.


### Goals

In this first session, we're going to port a small RNA sequencing pipelines to containers.  The pipeline is based on [RNAseq-NF](https://github.com/nextflow-io/rnaseq-nf) by the Nextflow developers.  
...
Our first task is to look for container images for this session.  Let's discuss in detail the case of Salmon.
...
This will make the script tidier and more readable; some features might even pop up, that were not evident at first.
...
> {: .source}
> 
> You'll then use this variable when you launch the container.
> 
> > ## Solution
> > 
...
> 
> Get rid of the occurrences of `-B` flags to bind mount additional host directories.  
> 
> In substitution, can you think of a single host directory path to bind mount, that includes all of the required directories?  
> Add an appropriate definition for the variable `SINGULARITY_BINDPATH`, just after the image paths definition at the beginning of the script.
> 
> > ## Solution
> > 
> > Note that picking any parent directory of the directory in this solution would work, too (just do not mount `/`, the container will crash).
> > 
> > ```
> > #!/bin/bash
> > 
...
If you had troubles, you can pick it up from `solutions/pipe.2.sh`
...
* defining the bind mount paths with `SINGULARITY_BINDPATH`;
...
There are ways of generalising this approach, that are beyond the scope of this session.