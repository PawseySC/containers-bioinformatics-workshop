...
You might know that every R installation ships with a utility to run scripts from the command line, so in this case we can go `Rscript home_rstudio/script.r`.  
