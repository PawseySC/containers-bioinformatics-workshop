...
This exercise is a bit more articulated.  You're going to build another image for samtools;  this time, though, you're not using Conda, you're compiling the tool yourself.  This will give the opportunity to touch on a set of important aspects.
...
Let's reply to these questions by using our latest build image, `sam:4`.


#### Upload an image to Docker Hub

We need a free Docker Hub account to perform this task.  If you don't have one, just relax and watch, won't be long.
...
In alternative, if you pushed the Docker image onto Docker hub, you can `pull` it from there and conversion will be automatically handled by Singularity:
...
Once the image is converted into the Singularity format:
...
1. **Condensate commands into few `RUN` instructions**
    ...
    Why is it so?  
    At one extreme, imagine one `RUN` per command.  Any edit to the same files in the container or any deletion of unnecessary files would create a separate layer, which means the final container is keeping a snapshot of each intermediate state, *e.g.* multiple versions of one file or even deleted files.  
    On the other extreme, concentrating every command in one `RUN` would minimise image size, as there would be one single layer for the whole set of files that makes up your containerised application.  As a disadvantage, readability of the Dockerfile would be reduced.
    
    In the end, the best practice is to group together commands that relate to the same component of the installation.  Readability is improved this way, and you would not gain space anyway if you further grouped them, as these groups of commands operate on different files.
    
    In our samtools example, you could for instance have one `RUN` for the `apt` installation of dependencies, and one `RUN` for samtools.
...
2. **Clean the installation process**
    ...
    These two removal operations, together with the `RUN` grouping from best practice `1.`, will reduce the final size of your samtools image by 30%!
    
    Note how this can be useful in the context of a Conda installation:
...
3. **Set useful environment variables**
...
5. **Think about the default command**
    ...
    You might be tempted to set the application binary as default, *e.g.* samtools, but in the end this is not a very useful setting.  The `CMD` default gets overridden by any command/argument you will add to the `run` commands, so you will need to state the main command anyway.  Moreover, lots of packages come with multiple executables, in which case some will be left out anyway
    ...
    For Python and R container, you might set it to the `python` and `R` interpreter, respectively, if you want.  
    Do not set `CMD` for RStudio and Jupyter images; these come with articulated setups that allow to spawn the web servers, so your choice would be ignored anyway.
...
6. **Abstract package versions if you can**
    ...
    For instance, in the samtools Dockerfile you could define a variable for the samtools version:
    ...
    A nice feature of `docker build` is that you can dynamically assign these `ARG` at build time, *e.g.* if you want to install samtools `1.8`:
